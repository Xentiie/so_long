...
t_color eval_item(t_rendering_data data)
{
    ...
    if (g_ame->player->player_pos.x == data.element_pos->x && g_ame->player->player_pos.y == data.element_pos->y)
    {
        destroy_graphics(data.self);
        g_ame->item_collected++;
        if (g_ame->item_collected == g_ame->item_count)
            add_entity(&update_exit, add_graphics(&eval_exit, 6, g_ame->exit_pos, v2(PIXEL_PER_TILE, PIXEL_PER_TILE)));
    }
    pos.x = data.element_pos->x / (float)g_ame->screen_dim->x;
    pos.y = data.element_pos->y / (float)g_ame->screen_dim->y;
    index = clamp(0, 14, (int)map(pnoise(pos.x, pos.y), v2_f(-1, 1), v2_f(0, 15)));
