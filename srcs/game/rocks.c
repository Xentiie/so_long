...
/*   Updated: 2022/05/17 15:03:59 by reclaire         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	eval_rocks(t_rendering_data data)
{
	...
	f = fabs(frac(sin(dot2(
						v2_f(data.element_pos->x, data.element_pos->y),
						v2_f(12.9898, 78.233))) * 43758.5453));
	if (f >= 0.0 && f < 0.33)
	...
	p = v2(lerp(0, curr->size->x - 1, data.local_pos.x),
			lerp(0, curr->size->y, data.local_pos.y));
